...
测试环境：13 张猫图，各 3 轮，`max_new_tokens=512`。长边自动限制 1024px，模型首次加载计入第 1 轮第 1 张。

| 模式 | 第 1 轮 | 第 2 轮 | 第 3 轮 | 平均 |
|------|---------|---------|---------|------|
| VLM + SAM2 | 5497ms | 4769ms | 4818ms | **5028ms** |
| VLM only | 4283ms | 4272ms | 4260ms | **4272ms** |

> 第 1 轮首张为冷启动（含 VLM + SAM2 模型加载）。SAM2 额外开销：+756ms (18%)。

**内存**：6 轮全程稳定在 9.8–13GB，每次检测后 MPS 内存回收生效。

**性能说明**：长边上限根据 GPU 显存自动选择（<12GB → 800px，12–16GB → 1024px，≥16GB → 1333px；MPS → 1024px）。CUDA 和 MPS 均在每次检测后执行激进显存/内存回收。
